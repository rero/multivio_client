...
/** @scope MyApp.DataSource.prototype */ {
  ...
  fetch: function(store, query) {
    ...
    if(SC.kindOf(record_type, Multivio.FileRecord) && parameters && parameters.url){
      /*
      SC.Request.getUrl('/server.test/get?url=%@'.fmt(parameters.url))
      .set('isJSON', YES)
    ...
    if(SC.kindOf(record_type, Multivio.SearchRecord) && parameters && parameters.url && parameters.query){
      var existingStoreKey = store.storeKeyExists(Multivio.SearchRecord, this._primaryKey(query));
      if(existingStoreKey){
        store.dataSourceDidFetchQuery(query);
        return YES;
        //return YES;
      }
      SC.Request.getUrl('/%@/document/search?angle=0&max_results=15&context_size=10&query=%@&url=%@'.fmt(this.get('serverName'), parameters.query, parameters.url))
      .set('isJSON', YES)
  ...
  _didFetchRecord: function(response, params) {
    ...
    if (SC.$ok(response)) {
      ...
      if(SC.kindOf(record_type, Multivio.ServerRecord)){
        store.loadRecords(Multivio.ServerRecord, [response.get('body')], [this._primaryKey(query)]);
      }
      if(SC.kindOf(record_type, Multivio.SearchRecord)){
      SC.Logger.debug('received search');
        store.loadRecords(Multivio.SearchRecord, [response.get('body')], [this._primaryKey(query)]);
      }
